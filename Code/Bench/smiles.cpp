...
#define BENCH_SMILES_TO_RDMOL(DATASET, SUFFIX, TAG)                            \
  TEST_CASE("SmilesToMol RDMol " SUFFIX, "[smiles][rdmol]" TAG) {              \
    const auto &smiles_list = bench_common::dataset_smiles(DATASET);           \
    SmilesParserParams params;                                                 \
    BENCHMARK("SmilesToMol RDMol " SUFFIX) {                                   \
      SmilesParseTemp temp;                                                    \
      auto total_atoms = 0;                                                    \
      for (const auto &smiles : smiles_list) {                                 \
        RDMol mol;                                                             \
        const bool success = SmilesToMol(smiles.c_str(), params, mol, temp);   \
        REQUIRE(success);                                                      \
        total_atoms += mol.getNumAtoms();                                      \
      }                                                                        \
      return total_atoms;                                                      \
...
#define BENCH_SMILES_TO_RDMOL_NOSAN(DATASET, SUFFIX, TAG)                      \
  TEST_CASE("SmilesToMol RDMol no-sanitize " SUFFIX, "[smiles][rdmol]" TAG) {  \
    const auto &smiles_list = bench_common::dataset_smiles(DATASET);           \
    SmilesParserParams params;                                                 \
    params.sanitize = false;                                                   \
    params.removeHs = false;                                                   \
    BENCHMARK("SmilesToMol RDMol no-sanitize " SUFFIX) {                       \
      SmilesParseTemp temp;                                                    \
      auto total_atoms = 0;                                                    \
      for (const auto &smiles : smiles_list) {                                 \
        RDMol mol;                                                             \
...
#define BENCH_SMILES_FOR(DATASET, SUFFIX, TAG)                                 \
  ...
  BENCH_SMILES_TO_RDMOL(DATASET, SUFFIX, TAG)                                  \
  BENCH_SMILES_TO_RDMOL_NOSAN(DATASET, SUFFIX, TAG)                            \
...
BENCH_SMILES_TO_MOL_NOSAN(Dataset::Organometallics, "organometallics",
                          "[organometallics]")
BENCH_SMILES_TO_RDMOL_NOSAN(Dataset::Organometallics, "organometallics",
                            "[organometallics]")

BENCH_SMILES_TO_MOL_NOSAN(Dataset::PreCanonicalNO2Azide, "pre_canonical_no2_azide",
                          "[pre_canonical_no2_azide]")
BENCH_SMILES_TO_RDMOL_NOSAN(Dataset::PreCanonicalNO2Azide,
                            "pre_canonical_no2_azide",
                            "[pre_canonical_no2_azide]")
...
BENCH_SMILES_TO_RDMOL(Dataset::KekulizeHard, "kekulize_hard", "[kekulize_hard]")
